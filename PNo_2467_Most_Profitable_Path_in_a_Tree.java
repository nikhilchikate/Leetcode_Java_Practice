...
class Solution {
    ...
    public int mostProfitablePath(int[][] edges, int bob, int[] amount) {
        ...
        dfs1(adj, 0, -1, 0, dis, disB, amount, 1);
        return mx;
    }

    public void dfs1(ArrayList<Integer> adj[], int s, int p, int pre, int dis[], int disB[], int amount[], int dd){
        if(disB[s]<dd)
            dis[s]= pre;
        else if(disB[s]== dd)
        ...
        for(int i= 0;i<adj[s].size();i++)
        {
            if(adj[s].get(i) == p)
                continue;
            dfs1(adj, adj[s].get(i), s, dis[s], dis, disB, amount, dd+1);
        }
        if(adj[s].size() == 1 && s!= 0)
